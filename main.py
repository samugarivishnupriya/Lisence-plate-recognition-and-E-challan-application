...
# displaying a button to browse a vehicle image
...
# displaying a button to detect the license plate and recognize the characters
...
# preprocess image using computer vision or opencv
def preprocess_image(image_path, resize=False):
    img = cv2.imread(image_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = img / 255 # converting into binary
    if resize:
        img = cv2.resize(img, (224, 224))
    return img


# sort_contours based on X-coordinate, so that characters are in order
...
# output - license plate recognition and detection , E-mail
def detect_and_recognize(file_path):
    ...
    for i in range(len(plot_image)):
        ...
        if i == 0:
            plt.imshow(plot_image[i]) # first colourful
        else:
            plt.imshow(plot_image[i], cmap="gray") # next all black and white
    # plt.show()
    plt.savefig(r"Plate_detect_and_recognize\outputs\preprocessing of number plate.png", dpi=300)
    ...
    plt.imshow(test_roi)
    # plt.show()
    plt.savefig(r"Plate_detect_and_recognize\outputs\grab_digit_contour.png", dpi=300)

    fig = plt.figure(figsize=(10, 5))  # segmented letter to print
    ...
    # plt.show()
    plt.savefig(r"Plate_detect_and_recognize\outputs\segmented_letter.png", dpi=300)
    ...
    final_string = ''
    for i, character in enumerate(crop_characters): # enumerate- returns both image and index
        fig.add_subplot(grid[i])
        title = np.array2string(predict_from_model(character, model, labels))
        plt.title('{}'.format(title.strip("'[]"), fontsize=20)) # strip- removes square box
    ...
    print(final_string)
    # plt.show()
    plt.savefig(r"Plate_detect_and_recognize\outputs\final_result.png", dpi=300)

    with open(r"Plate_detect_and_recognize\outputs\car number text.txt", "a") as op_file:
    ...
    uploaded = Image.open(r"Plate_detect_and_recognize\outputs\final_result.png")
...
# creating a connection with vehicle database
def get_connection():
    connection = sqlite3.connect('vehicledatabase.db')
    return connection


# loading wpod-net pretrained model
def load_model(path):
    try:
        path = splitext(path)[0] # only name will be taken
        with open('%s.json' % path, 'r') as json_file: # we open the file
...
if __name__ == '__main__':
    ...
    # creating label for the roll numbers
